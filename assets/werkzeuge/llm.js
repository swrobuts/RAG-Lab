...
const L = {
  ...
  keinServer: { de: 'Keine Verbindung. Läuft der Server? Ist CORS eingeschaltet (LM Studio: „Enable CORS“ in den Server-Einstellungen, wirkt erst nach Neustart des Servers, oder lms server start --cors; Ollama: OLLAMA_ORIGINS)? Chrome fragt bei der ersten Anfrage neben der Adresszeile nach der Erlaubnis für das lokale Netzwerk („Local Network Access“) – bis zum Klick auf „Zulassen“ wartet die Anfrage. Safari blockiert Aufrufe an http://localhost von einer HTTPS-Seite (und im Modus „Nur HTTPS“ jede HTTP-Adresse); nutzen Sie Chrome, Edge oder Firefox.', en: 'No connection. Is the server running? Is CORS enabled (LM Studio: “Enable CORS” in the server settings, effective only after restarting the server, or lms server start --cors; Ollama: OLLAMA_ORIGINS)? Chrome asks next to the address bar for local network permission (“Local Network Access”) on the first request – the request waits until you click “Allow”. Safari blocks calls to http://localhost from an HTTPS page (and, in “HTTPS-only” mode, any HTTP address); use Chrome, Edge or Firefox.' },
