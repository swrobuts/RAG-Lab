...
const L = {
  ...
  keinServer: { de: 'Keine Verbindung. Läuft der Server? Ist CORS eingeschaltet (LM Studio: „Enable CORS“ in den Server-Einstellungen oder lms server start --cors; Ollama: OLLAMA_ORIGINS)? Safari blockiert Aufrufe an http://localhost von einer HTTPS-Seite; nutzen Sie Chrome, Edge oder Firefox oder öffnen Sie das Lab lokal.', en: 'No connection. Is the server running? Is CORS enabled (LM Studio: “Enable CORS” in the server settings or lms server start --cors; Ollama: OLLAMA_ORIGINS)? Safari blocks calls to http://localhost from an HTTPS page; use Chrome, Edge or Firefox, or open the lab locally.' },
