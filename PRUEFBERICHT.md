...
- `data/eval/pageindex-baseline.json` und `pageindex.json` (kopiert), `data/betrieb.json`
  neu erzeugt (37 Commits, Block `pageindex` mit Knotenzahl und Voreinstellungen).
...
- `tools/verify.mjs`: 25 neue Werte (`eval.pageindex.*`, `eval.pageindexAlt.*`,
  `pageindex.*`, `commits.pageindex`) und zwei Strukturprüfungen der Protokolle; `npm run verify`
  1038 Prüfungen ohne Befund, `tools/pruefung/texte.mjs` unverändert 8 Mustertreffer
